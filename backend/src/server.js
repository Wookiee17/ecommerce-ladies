...
app.use(cors({
  origin: function (origin, callback) {
    const allowedOrigins = [process.env.FRONTEND_URL, 'http://localhost:5173', 'http://localhost:3000', 'https://evara-com.netlify.app'];
...
// Rate limiting - skip for image serving
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 1000, // limit each IP to 1000 requests per windowMs
  message: 'Too many requests from this IP, please try again later.',
  skip: (req) => req.path.startsWith('/api/images/') // Skip rate limit for images
});
app.use('/api/', limiter);
...
app.get('/health', (req, res) => {
  res.json({
    ...
    version: '1.0.1', // Added Binary fix for images
...
app.listen(PORT, () => {
  ...
  console.log('🔄 Last Updated: Build 1052');
